...
for i in range (len(covid_poll_approval.index)):
    if (covid_poll_approval["party"][i] == "D") and ((math.isnan(covid_poll_approval["sample_size"][i])) is not True):
...
plt.title('Approval Vs Disapproval by month: Democrat')

plt.xticks(ind + width / 4, ('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'))
plt.legend(loc='best')
print(plt.show())